...
const fsLightbox = {
    componentsServices: {
        isFullscreenOpenManager: { get: () => false },
        isLightboxOpenManager: { set: jest.fn() }
    },
    core: {
    ...
    elements: { container: { classList: { add: jest.fn(), remove: jest.fn() } } },
    slideSwipingProps: {},
};
...
test('before fadeOut (instant actions)', () => {
    ...
    fsLightbox.componentsServices.isFullscreenOpenManager.get = () => true;
    lightboxCloseActions.runActions();
...
test('after fade out', () => {
    ...
    expect(fsLightbox.componentsServices.isLightboxOpenManager.set).toBeCalledWith(false);
